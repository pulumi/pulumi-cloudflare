# coding=utf-8
# *** WARNING: this file was generated by pulumi-language-python. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import builtins as _builtins
import warnings
import sys
import pulumi
import pulumi.runtime
from typing import Any, Mapping, Optional, Sequence, Union, overload
if sys.version_info >= (3, 11):
    from typing import NotRequired, TypedDict, TypeAlias
else:
    from typing_extensions import NotRequired, TypedDict, TypeAlias
from .. import _utilities
from . import outputs

__all__ = [
    'GetTestsResult',
    'AwaitableGetTestsResult',
    'get_tests',
    'get_tests_output',
]

@pulumi.output_type
class GetTestsResult:
    """
    A collection of values returned by getTests.
    """
    def __init__(__self__, account_id=None, kind=None, max_items=None, results=None, test_name=None):
        if account_id and not isinstance(account_id, str):
            raise TypeError("Expected argument 'account_id' to be a str")
        pulumi.set(__self__, "account_id", account_id)
        if kind and not isinstance(kind, str):
            raise TypeError("Expected argument 'kind' to be a str")
        pulumi.set(__self__, "kind", kind)
        if max_items and not isinstance(max_items, int):
            raise TypeError("Expected argument 'max_items' to be a int")
        pulumi.set(__self__, "max_items", max_items)
        if results and not isinstance(results, list):
            raise TypeError("Expected argument 'results' to be a list")
        pulumi.set(__self__, "results", results)
        if test_name and not isinstance(test_name, str):
            raise TypeError("Expected argument 'test_name' to be a str")
        pulumi.set(__self__, "test_name", test_name)

    @_builtins.property
    @pulumi.getter(name="accountId")
    def account_id(self) -> Optional[_builtins.str]:
        """
        Unique identifier linked to an account.
        """
        return pulumi.get(self, "account_id")

    @_builtins.property
    @pulumi.getter
    def kind(self) -> Optional[_builtins.str]:
        """
        Filter by test type.
        Available values: "http", "traceroute".
        """
        return pulumi.get(self, "kind")

    @_builtins.property
    @pulumi.getter(name="maxItems")
    def max_items(self) -> Optional[_builtins.int]:
        """
        Max items to fetch, default: 1000
        """
        return pulumi.get(self, "max_items")

    @_builtins.property
    @pulumi.getter
    def results(self) -> Sequence['outputs.GetTestsResultResult']:
        """
        The items returned by the data source
        """
        return pulumi.get(self, "results")

    @_builtins.property
    @pulumi.getter(name="testName")
    def test_name(self) -> Optional[_builtins.str]:
        """
        Filter by test name.
        """
        return pulumi.get(self, "test_name")


class AwaitableGetTestsResult(GetTestsResult):
    # pylint: disable=using-constant-test
    def __await__(self):
        if False:
            yield self
        return GetTestsResult(
            account_id=self.account_id,
            kind=self.kind,
            max_items=self.max_items,
            results=self.results,
            test_name=self.test_name)


def get_tests(account_id: Optional[_builtins.str] = None,
              kind: Optional[_builtins.str] = None,
              max_items: Optional[_builtins.int] = None,
              test_name: Optional[_builtins.str] = None,
              opts: Optional[pulumi.InvokeOptions] = None) -> AwaitableGetTestsResult:
    """
    Accepted Permissions

    - `Cloudflare DEX Read`
    - `Cloudflare DEX Write`
    - `Zero Trust Read`
    - `Zero Trust Report`

    ## Example Usage

    ```python
    import pulumi
    import pulumi_cloudflare as cloudflare

    example_zero_trust_dex_tests = cloudflare.zerotrustdex.get_tests(account_id="01a7362d577a6c3019a474fd6f485823",
        kind="http",
        test_name="testName")
    ```


    :param _builtins.str account_id: Unique identifier linked to an account.
    :param _builtins.str kind: Filter by test type.
           Available values: "http", "traceroute".
    :param _builtins.int max_items: Max items to fetch, default: 1000
    :param _builtins.str test_name: Filter by test name.
    """
    __args__ = dict()
    __args__['accountId'] = account_id
    __args__['kind'] = kind
    __args__['maxItems'] = max_items
    __args__['testName'] = test_name
    opts = pulumi.InvokeOptions.merge(_utilities.get_invoke_opts_defaults(), opts)
    __ret__ = pulumi.runtime.invoke('cloudflare:zeroTrustDex/tests:getTests', __args__, opts=opts, typ=GetTestsResult).value

    return AwaitableGetTestsResult(
        account_id=pulumi.get(__ret__, 'account_id'),
        kind=pulumi.get(__ret__, 'kind'),
        max_items=pulumi.get(__ret__, 'max_items'),
        results=pulumi.get(__ret__, 'results'),
        test_name=pulumi.get(__ret__, 'test_name'))
def get_tests_output(account_id: pulumi.Input[Optional[Optional[_builtins.str]]] = None,
                     kind: pulumi.Input[Optional[Optional[_builtins.str]]] = None,
                     max_items: pulumi.Input[Optional[Optional[_builtins.int]]] = None,
                     test_name: pulumi.Input[Optional[Optional[_builtins.str]]] = None,
                     opts: Optional[Union[pulumi.InvokeOptions, pulumi.InvokeOutputOptions]] = None) -> pulumi.Output[GetTestsResult]:
    """
    Accepted Permissions

    - `Cloudflare DEX Read`
    - `Cloudflare DEX Write`
    - `Zero Trust Read`
    - `Zero Trust Report`

    ## Example Usage

    ```python
    import pulumi
    import pulumi_cloudflare as cloudflare

    example_zero_trust_dex_tests = cloudflare.zerotrustdex.get_tests(account_id="01a7362d577a6c3019a474fd6f485823",
        kind="http",
        test_name="testName")
    ```


    :param _builtins.str account_id: Unique identifier linked to an account.
    :param _builtins.str kind: Filter by test type.
           Available values: "http", "traceroute".
    :param _builtins.int max_items: Max items to fetch, default: 1000
    :param _builtins.str test_name: Filter by test name.
    """
    __args__ = dict()
    __args__['accountId'] = account_id
    __args__['kind'] = kind
    __args__['maxItems'] = max_items
    __args__['testName'] = test_name
    opts = pulumi.InvokeOutputOptions.merge(_utilities.get_invoke_opts_defaults(), opts)
    __ret__ = pulumi.runtime.invoke_output('cloudflare:zeroTrustDex/tests:getTests', __args__, opts=opts, typ=GetTestsResult)
    return __ret__.apply(lambda __response__: GetTestsResult(
        account_id=pulumi.get(__response__, 'account_id'),
        kind=pulumi.get(__response__, 'kind'),
        max_items=pulumi.get(__response__, 'max_items'),
        results=pulumi.get(__response__, 'results'),
        test_name=pulumi.get(__response__, 'test_name')))
