# coding=utf-8
# *** WARNING: this file was generated by pulumi-language-python. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import builtins as _builtins
import warnings
import sys
import pulumi
import pulumi.runtime
from typing import Any, Mapping, Optional, Sequence, Union, overload
if sys.version_info >= (3, 11):
    from typing import NotRequired, TypedDict, TypeAlias
else:
    from typing_extensions import NotRequired, TypedDict, TypeAlias
from .. import _utilities

__all__ = [
    'GetContentScanningResult',
    'AwaitableGetContentScanningResult',
    'get_content_scanning',
    'get_content_scanning_output',
]

@pulumi.output_type
class GetContentScanningResult:
    """
    A collection of values returned by getContentScanning.
    """
    def __init__(__self__, modified=None, value=None, zone_id=None):
        if modified and not isinstance(modified, str):
            raise TypeError("Expected argument 'modified' to be a str")
        pulumi.set(__self__, "modified", modified)
        if value and not isinstance(value, str):
            raise TypeError("Expected argument 'value' to be a str")
        pulumi.set(__self__, "value", value)
        if zone_id and not isinstance(zone_id, str):
            raise TypeError("Expected argument 'zone_id' to be a str")
        pulumi.set(__self__, "zone_id", zone_id)

    @_builtins.property
    @pulumi.getter
    def modified(self) -> _builtins.str:
        """
        Defines the last modification date (ISO 8601) of the Content Scanning status.
        """
        return pulumi.get(self, "modified")

    @_builtins.property
    @pulumi.getter
    def value(self) -> _builtins.str:
        """
        Defines the status of Content Scanning.
        """
        return pulumi.get(self, "value")

    @_builtins.property
    @pulumi.getter(name="zoneId")
    def zone_id(self) -> Optional[_builtins.str]:
        """
        Defines an identifier.
        """
        return pulumi.get(self, "zone_id")


class AwaitableGetContentScanningResult(GetContentScanningResult):
    # pylint: disable=using-constant-test
    def __await__(self):
        if False:
            yield self
        return GetContentScanningResult(
            modified=self.modified,
            value=self.value,
            zone_id=self.zone_id)


def get_content_scanning(zone_id: Optional[_builtins.str] = None,
                         opts: Optional[pulumi.InvokeOptions] = None) -> AwaitableGetContentScanningResult:
    """
    Accepted Permissions

    - `Account WAF Read`
    - `Account WAF Write`
    - `Zone WAF Read`
    - `Zone WAF Write`

    ## Example Usage

    ```python
    import pulumi
    import pulumi_cloudflare as cloudflare

    example_content_scanning = cloudflare.contentscanning.get_content_scanning(zone_id="023e105f4ecef8ad9ca31a8372d0c353")
    ```


    :param _builtins.str zone_id: Defines an identifier.
    """
    __args__ = dict()
    __args__['zoneId'] = zone_id
    opts = pulumi.InvokeOptions.merge(_utilities.get_invoke_opts_defaults(), opts)
    __ret__ = pulumi.runtime.invoke('cloudflare:contentScanning/contentScanning:getContentScanning', __args__, opts=opts, typ=GetContentScanningResult).value

    return AwaitableGetContentScanningResult(
        modified=pulumi.get(__ret__, 'modified'),
        value=pulumi.get(__ret__, 'value'),
        zone_id=pulumi.get(__ret__, 'zone_id'))
def get_content_scanning_output(zone_id: pulumi.Input[Optional[Optional[_builtins.str]]] = None,
                                opts: Optional[Union[pulumi.InvokeOptions, pulumi.InvokeOutputOptions]] = None) -> pulumi.Output[GetContentScanningResult]:
    """
    Accepted Permissions

    - `Account WAF Read`
    - `Account WAF Write`
    - `Zone WAF Read`
    - `Zone WAF Write`

    ## Example Usage

    ```python
    import pulumi
    import pulumi_cloudflare as cloudflare

    example_content_scanning = cloudflare.contentscanning.get_content_scanning(zone_id="023e105f4ecef8ad9ca31a8372d0c353")
    ```


    :param _builtins.str zone_id: Defines an identifier.
    """
    __args__ = dict()
    __args__['zoneId'] = zone_id
    opts = pulumi.InvokeOutputOptions.merge(_utilities.get_invoke_opts_defaults(), opts)
    __ret__ = pulumi.runtime.invoke_output('cloudflare:contentScanning/contentScanning:getContentScanning', __args__, opts=opts, typ=GetContentScanningResult)
    return __ret__.apply(lambda __response__: GetContentScanningResult(
        modified=pulumi.get(__response__, 'modified'),
        value=pulumi.get(__response__, 'value'),
        zone_id=pulumi.get(__response__, 'zone_id')))
