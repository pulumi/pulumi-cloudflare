// *** WARNING: this file was generated by pulumi-language-java. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.pulumi.cloudflare.zeroTrustDns.inputs;

import com.pulumi.cloudflare.zeroTrustDns.inputs.LocationEndpointsDohNetworkArgs;
import com.pulumi.core.Output;
import com.pulumi.core.annotations.Import;
import java.lang.Boolean;
import java.util.List;
import java.util.Objects;
import java.util.Optional;
import javax.annotation.Nullable;


public final class LocationEndpointsDohArgs extends com.pulumi.resources.ResourceArgs {

    public static final LocationEndpointsDohArgs Empty = new LocationEndpointsDohArgs();

    /**
     * Indicate whether the DOH endpoint is enabled for this location.
     * 
     */
    @Import(name="enabled")
    private @Nullable Output<Boolean> enabled;

    /**
     * @return Indicate whether the DOH endpoint is enabled for this location.
     * 
     */
    public Optional<Output<Boolean>> enabled() {
        return Optional.ofNullable(this.enabled);
    }

    /**
     * Specify the list of allowed source IP network ranges for this endpoint. When the list is empty, the endpoint allows all source IPs. The list takes effect only if the endpoint is enabled for this location.
     * 
     */
    @Import(name="networks")
    private @Nullable Output<List<LocationEndpointsDohNetworkArgs>> networks;

    /**
     * @return Specify the list of allowed source IP network ranges for this endpoint. When the list is empty, the endpoint allows all source IPs. The list takes effect only if the endpoint is enabled for this location.
     * 
     */
    public Optional<Output<List<LocationEndpointsDohNetworkArgs>>> networks() {
        return Optional.ofNullable(this.networks);
    }

    /**
     * Specify whether the DOH endpoint requires user identity authentication.
     * 
     */
    @Import(name="requireToken")
    private @Nullable Output<Boolean> requireToken;

    /**
     * @return Specify whether the DOH endpoint requires user identity authentication.
     * 
     */
    public Optional<Output<Boolean>> requireToken() {
        return Optional.ofNullable(this.requireToken);
    }

    private LocationEndpointsDohArgs() {}

    private LocationEndpointsDohArgs(LocationEndpointsDohArgs $) {
        this.enabled = $.enabled;
        this.networks = $.networks;
        this.requireToken = $.requireToken;
    }

    public static Builder builder() {
        return new Builder();
    }
    public static Builder builder(LocationEndpointsDohArgs defaults) {
        return new Builder(defaults);
    }

    public static final class Builder {
        private LocationEndpointsDohArgs $;

        public Builder() {
            $ = new LocationEndpointsDohArgs();
        }

        public Builder(LocationEndpointsDohArgs defaults) {
            $ = new LocationEndpointsDohArgs(Objects.requireNonNull(defaults));
        }

        /**
         * @param enabled Indicate whether the DOH endpoint is enabled for this location.
         * 
         * @return builder
         * 
         */
        public Builder enabled(@Nullable Output<Boolean> enabled) {
            $.enabled = enabled;
            return this;
        }

        /**
         * @param enabled Indicate whether the DOH endpoint is enabled for this location.
         * 
         * @return builder
         * 
         */
        public Builder enabled(Boolean enabled) {
            return enabled(Output.of(enabled));
        }

        /**
         * @param networks Specify the list of allowed source IP network ranges for this endpoint. When the list is empty, the endpoint allows all source IPs. The list takes effect only if the endpoint is enabled for this location.
         * 
         * @return builder
         * 
         */
        public Builder networks(@Nullable Output<List<LocationEndpointsDohNetworkArgs>> networks) {
            $.networks = networks;
            return this;
        }

        /**
         * @param networks Specify the list of allowed source IP network ranges for this endpoint. When the list is empty, the endpoint allows all source IPs. The list takes effect only if the endpoint is enabled for this location.
         * 
         * @return builder
         * 
         */
        public Builder networks(List<LocationEndpointsDohNetworkArgs> networks) {
            return networks(Output.of(networks));
        }

        /**
         * @param networks Specify the list of allowed source IP network ranges for this endpoint. When the list is empty, the endpoint allows all source IPs. The list takes effect only if the endpoint is enabled for this location.
         * 
         * @return builder
         * 
         */
        public Builder networks(LocationEndpointsDohNetworkArgs... networks) {
            return networks(List.of(networks));
        }

        /**
         * @param requireToken Specify whether the DOH endpoint requires user identity authentication.
         * 
         * @return builder
         * 
         */
        public Builder requireToken(@Nullable Output<Boolean> requireToken) {
            $.requireToken = requireToken;
            return this;
        }

        /**
         * @param requireToken Specify whether the DOH endpoint requires user identity authentication.
         * 
         * @return builder
         * 
         */
        public Builder requireToken(Boolean requireToken) {
            return requireToken(Output.of(requireToken));
        }

        public LocationEndpointsDohArgs build() {
            return $;
        }
    }

}
