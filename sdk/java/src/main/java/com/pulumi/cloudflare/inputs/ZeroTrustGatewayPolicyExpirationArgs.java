// *** WARNING: this file was generated by pulumi-language-java. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.pulumi.cloudflare.inputs;

import com.pulumi.core.Output;
import com.pulumi.core.annotations.Import;
import com.pulumi.exceptions.MissingRequiredPropertyException;
import java.lang.Boolean;
import java.lang.Integer;
import java.lang.String;
import java.util.Objects;
import java.util.Optional;
import javax.annotation.Nullable;


public final class ZeroTrustGatewayPolicyExpirationArgs extends com.pulumi.resources.ResourceArgs {

    public static final ZeroTrustGatewayPolicyExpirationArgs Empty = new ZeroTrustGatewayPolicyExpirationArgs();

    /**
     * The default duration a policy will be active in minutes. Must be set in order to use the `reset_expiration` endpoint on this rule.
     * 
     */
    @Import(name="duration")
    private @Nullable Output<Integer> duration;

    /**
     * @return The default duration a policy will be active in minutes. Must be set in order to use the `reset_expiration` endpoint on this rule.
     * 
     */
    public Optional<Output<Integer>> duration() {
        return Optional.ofNullable(this.duration);
    }

    /**
     * Whether the policy has expired.
     * 
     */
    @Import(name="expired")
    private @Nullable Output<Boolean> expired;

    /**
     * @return Whether the policy has expired.
     * 
     */
    public Optional<Output<Boolean>> expired() {
        return Optional.ofNullable(this.expired);
    }

    /**
     * The time stamp at which the policy will expire and cease to be
     * applied.
     * 
     */
    @Import(name="expiresAt", required=true)
    private Output<String> expiresAt;

    /**
     * @return The time stamp at which the policy will expire and cease to be
     * applied.
     * 
     */
    public Output<String> expiresAt() {
        return this.expiresAt;
    }

    private ZeroTrustGatewayPolicyExpirationArgs() {}

    private ZeroTrustGatewayPolicyExpirationArgs(ZeroTrustGatewayPolicyExpirationArgs $) {
        this.duration = $.duration;
        this.expired = $.expired;
        this.expiresAt = $.expiresAt;
    }

    public static Builder builder() {
        return new Builder();
    }
    public static Builder builder(ZeroTrustGatewayPolicyExpirationArgs defaults) {
        return new Builder(defaults);
    }

    public static final class Builder {
        private ZeroTrustGatewayPolicyExpirationArgs $;

        public Builder() {
            $ = new ZeroTrustGatewayPolicyExpirationArgs();
        }

        public Builder(ZeroTrustGatewayPolicyExpirationArgs defaults) {
            $ = new ZeroTrustGatewayPolicyExpirationArgs(Objects.requireNonNull(defaults));
        }

        /**
         * @param duration The default duration a policy will be active in minutes. Must be set in order to use the `reset_expiration` endpoint on this rule.
         * 
         * @return builder
         * 
         */
        public Builder duration(@Nullable Output<Integer> duration) {
            $.duration = duration;
            return this;
        }

        /**
         * @param duration The default duration a policy will be active in minutes. Must be set in order to use the `reset_expiration` endpoint on this rule.
         * 
         * @return builder
         * 
         */
        public Builder duration(Integer duration) {
            return duration(Output.of(duration));
        }

        /**
         * @param expired Whether the policy has expired.
         * 
         * @return builder
         * 
         */
        public Builder expired(@Nullable Output<Boolean> expired) {
            $.expired = expired;
            return this;
        }

        /**
         * @param expired Whether the policy has expired.
         * 
         * @return builder
         * 
         */
        public Builder expired(Boolean expired) {
            return expired(Output.of(expired));
        }

        /**
         * @param expiresAt The time stamp at which the policy will expire and cease to be
         * applied.
         * 
         * @return builder
         * 
         */
        public Builder expiresAt(Output<String> expiresAt) {
            $.expiresAt = expiresAt;
            return this;
        }

        /**
         * @param expiresAt The time stamp at which the policy will expire and cease to be
         * applied.
         * 
         * @return builder
         * 
         */
        public Builder expiresAt(String expiresAt) {
            return expiresAt(Output.of(expiresAt));
        }

        public ZeroTrustGatewayPolicyExpirationArgs build() {
            if ($.expiresAt == null) {
                throw new MissingRequiredPropertyException("ZeroTrustGatewayPolicyExpirationArgs", "expiresAt");
            }
            return $;
        }
    }

}
