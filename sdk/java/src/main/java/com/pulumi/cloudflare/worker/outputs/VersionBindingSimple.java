// *** WARNING: this file was generated by pulumi-language-java. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.pulumi.cloudflare.worker.outputs;

import com.pulumi.core.annotations.CustomType;
import com.pulumi.exceptions.MissingRequiredPropertyException;
import java.lang.Double;
import java.lang.Integer;
import java.util.Objects;
import java.util.Optional;
import javax.annotation.Nullable;

@CustomType
public final class VersionBindingSimple {
    /**
     * @return The limit (requests per period).
     * 
     */
    private Double limit;
    /**
     * @return Duration in seconds to apply the mitigation action after the rate limit is exceeded. Valid values are 0 (disabled), 10, or multiples of 60 up to 86400. Must be greater than or equal to the period when non-zero.
     * 
     */
    private @Nullable Integer mitigationTimeout;
    /**
     * @return The period in seconds.
     * 
     */
    private Integer period;

    private VersionBindingSimple() {}
    /**
     * @return The limit (requests per period).
     * 
     */
    public Double limit() {
        return this.limit;
    }
    /**
     * @return Duration in seconds to apply the mitigation action after the rate limit is exceeded. Valid values are 0 (disabled), 10, or multiples of 60 up to 86400. Must be greater than or equal to the period when non-zero.
     * 
     */
    public Optional<Integer> mitigationTimeout() {
        return Optional.ofNullable(this.mitigationTimeout);
    }
    /**
     * @return The period in seconds.
     * 
     */
    public Integer period() {
        return this.period;
    }

    public static Builder builder() {
        return new Builder();
    }

    public static Builder builder(VersionBindingSimple defaults) {
        return new Builder(defaults);
    }
    @CustomType.Builder
    public static final class Builder {
        private Double limit;
        private @Nullable Integer mitigationTimeout;
        private Integer period;
        public Builder() {}
        public Builder(VersionBindingSimple defaults) {
    	      Objects.requireNonNull(defaults);
    	      this.limit = defaults.limit;
    	      this.mitigationTimeout = defaults.mitigationTimeout;
    	      this.period = defaults.period;
        }

        @CustomType.Setter
        public Builder limit(Double limit) {
            if (limit == null) {
              throw new MissingRequiredPropertyException("VersionBindingSimple", "limit");
            }
            this.limit = limit;
            return this;
        }
        @CustomType.Setter
        public Builder mitigationTimeout(@Nullable Integer mitigationTimeout) {

            this.mitigationTimeout = mitigationTimeout;
            return this;
        }
        @CustomType.Setter
        public Builder period(Integer period) {
            if (period == null) {
              throw new MissingRequiredPropertyException("VersionBindingSimple", "period");
            }
            this.period = period;
            return this;
        }
        public VersionBindingSimple build() {
            final var _resultValue = new VersionBindingSimple();
            _resultValue.limit = limit;
            _resultValue.mitigationTimeout = mitigationTimeout;
            _resultValue.period = period;
            return _resultValue;
        }
    }
}
