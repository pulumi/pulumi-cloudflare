// *** WARNING: this file was generated by pulumi-language-java. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.pulumi.cloudflare.workersCustom.inputs;

import com.pulumi.core.annotations.Import;
import java.lang.String;
import java.util.Objects;
import java.util.Optional;
import javax.annotation.Nullable;


public final class GetDomainFilter extends com.pulumi.resources.InvokeArgs {

    public static final GetDomainFilter Empty = new GetDomainFilter();

    /**
     * Worker environment associated with the domain.
     * 
     */
    @Import(name="environment")
    private @Nullable String environment;

    /**
     * @return Worker environment associated with the domain.
     * 
     */
    public Optional<String> environment() {
        return Optional.ofNullable(this.environment);
    }

    /**
     * Hostname of the domain.
     * 
     */
    @Import(name="hostname")
    private @Nullable String hostname;

    /**
     * @return Hostname of the domain.
     * 
     */
    public Optional<String> hostname() {
        return Optional.ofNullable(this.hostname);
    }

    /**
     * Name of the Worker associated with the domain.
     * 
     */
    @Import(name="service")
    private @Nullable String service;

    /**
     * @return Name of the Worker associated with the domain.
     * 
     */
    public Optional<String> service() {
        return Optional.ofNullable(this.service);
    }

    /**
     * ID of the zone containing the domain hostname.
     * 
     */
    @Import(name="zoneId")
    private @Nullable String zoneId;

    /**
     * @return ID of the zone containing the domain hostname.
     * 
     */
    public Optional<String> zoneId() {
        return Optional.ofNullable(this.zoneId);
    }

    /**
     * Name of the zone containing the domain hostname.
     * 
     */
    @Import(name="zoneName")
    private @Nullable String zoneName;

    /**
     * @return Name of the zone containing the domain hostname.
     * 
     */
    public Optional<String> zoneName() {
        return Optional.ofNullable(this.zoneName);
    }

    private GetDomainFilter() {}

    private GetDomainFilter(GetDomainFilter $) {
        this.environment = $.environment;
        this.hostname = $.hostname;
        this.service = $.service;
        this.zoneId = $.zoneId;
        this.zoneName = $.zoneName;
    }

    public static Builder builder() {
        return new Builder();
    }
    public static Builder builder(GetDomainFilter defaults) {
        return new Builder(defaults);
    }

    public static final class Builder {
        private GetDomainFilter $;

        public Builder() {
            $ = new GetDomainFilter();
        }

        public Builder(GetDomainFilter defaults) {
            $ = new GetDomainFilter(Objects.requireNonNull(defaults));
        }

        /**
         * @param environment Worker environment associated with the domain.
         * 
         * @return builder
         * 
         */
        public Builder environment(@Nullable String environment) {
            $.environment = environment;
            return this;
        }

        /**
         * @param hostname Hostname of the domain.
         * 
         * @return builder
         * 
         */
        public Builder hostname(@Nullable String hostname) {
            $.hostname = hostname;
            return this;
        }

        /**
         * @param service Name of the Worker associated with the domain.
         * 
         * @return builder
         * 
         */
        public Builder service(@Nullable String service) {
            $.service = service;
            return this;
        }

        /**
         * @param zoneId ID of the zone containing the domain hostname.
         * 
         * @return builder
         * 
         */
        public Builder zoneId(@Nullable String zoneId) {
            $.zoneId = zoneId;
            return this;
        }

        /**
         * @param zoneName Name of the zone containing the domain hostname.
         * 
         * @return builder
         * 
         */
        public Builder zoneName(@Nullable String zoneName) {
            $.zoneName = zoneName;
            return this;
        }

        public GetDomainFilter build() {
            return $;
        }
    }

}
