// *** WARNING: this file was generated by pulumi-language-java. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.pulumi.cloudflare.zeroTrustAccessCustom.outputs;

import com.pulumi.core.annotations.CustomType;
import com.pulumi.exceptions.MissingRequiredPropertyException;
import java.lang.String;
import java.util.Objects;
import java.util.Optional;
import javax.annotation.Nullable;

@CustomType
public final class GetPageResult {
    /**
     * @return Identifier.
     * 
     */
    private @Nullable String accountId;
    /**
     * @return Custom page HTML.
     * 
     */
    private String customHtml;
    /**
     * @return UUID.
     * 
     */
    private String customPageId;
    /**
     * @return UUID.
     * 
     */
    private String id;
    /**
     * @return Custom page name.
     * 
     */
    private String name;
    /**
     * @return Custom page type.
     * Available values: &#34;identityDenied&#34;, &#34;forbidden&#34;.
     * 
     */
    private String type;
    /**
     * @return UUID.
     * 
     */
    private String uid;

    private GetPageResult() {}
    /**
     * @return Identifier.
     * 
     */
    public Optional<String> accountId() {
        return Optional.ofNullable(this.accountId);
    }
    /**
     * @return Custom page HTML.
     * 
     */
    public String customHtml() {
        return this.customHtml;
    }
    /**
     * @return UUID.
     * 
     */
    public String customPageId() {
        return this.customPageId;
    }
    /**
     * @return UUID.
     * 
     */
    public String id() {
        return this.id;
    }
    /**
     * @return Custom page name.
     * 
     */
    public String name() {
        return this.name;
    }
    /**
     * @return Custom page type.
     * Available values: &#34;identityDenied&#34;, &#34;forbidden&#34;.
     * 
     */
    public String type() {
        return this.type;
    }
    /**
     * @return UUID.
     * 
     */
    public String uid() {
        return this.uid;
    }

    public static Builder builder() {
        return new Builder();
    }

    public static Builder builder(GetPageResult defaults) {
        return new Builder(defaults);
    }
    @CustomType.Builder
    public static final class Builder {
        private @Nullable String accountId;
        private String customHtml;
        private String customPageId;
        private String id;
        private String name;
        private String type;
        private String uid;
        public Builder() {}
        public Builder(GetPageResult defaults) {
    	      Objects.requireNonNull(defaults);
    	      this.accountId = defaults.accountId;
    	      this.customHtml = defaults.customHtml;
    	      this.customPageId = defaults.customPageId;
    	      this.id = defaults.id;
    	      this.name = defaults.name;
    	      this.type = defaults.type;
    	      this.uid = defaults.uid;
        }

        @CustomType.Setter
        public Builder accountId(@Nullable String accountId) {

            this.accountId = accountId;
            return this;
        }
        @CustomType.Setter
        public Builder customHtml(String customHtml) {
            if (customHtml == null) {
              throw new MissingRequiredPropertyException("GetPageResult", "customHtml");
            }
            this.customHtml = customHtml;
            return this;
        }
        @CustomType.Setter
        public Builder customPageId(String customPageId) {
            if (customPageId == null) {
              throw new MissingRequiredPropertyException("GetPageResult", "customPageId");
            }
            this.customPageId = customPageId;
            return this;
        }
        @CustomType.Setter
        public Builder id(String id) {
            if (id == null) {
              throw new MissingRequiredPropertyException("GetPageResult", "id");
            }
            this.id = id;
            return this;
        }
        @CustomType.Setter
        public Builder name(String name) {
            if (name == null) {
              throw new MissingRequiredPropertyException("GetPageResult", "name");
            }
            this.name = name;
            return this;
        }
        @CustomType.Setter
        public Builder type(String type) {
            if (type == null) {
              throw new MissingRequiredPropertyException("GetPageResult", "type");
            }
            this.type = type;
            return this;
        }
        @CustomType.Setter
        public Builder uid(String uid) {
            if (uid == null) {
              throw new MissingRequiredPropertyException("GetPageResult", "uid");
            }
            this.uid = uid;
            return this;
        }
        public GetPageResult build() {
            final var _resultValue = new GetPageResult();
            _resultValue.accountId = accountId;
            _resultValue.customHtml = customHtml;
            _resultValue.customPageId = customPageId;
            _resultValue.id = id;
            _resultValue.name = name;
            _resultValue.type = type;
            _resultValue.uid = uid;
            return _resultValue;
        }
    }
}
