// *** WARNING: this file was generated by pulumi-language-java. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.pulumi.cloudflare.certificate.inputs;

import com.pulumi.core.Output;
import com.pulumi.core.annotations.Import;
import java.lang.String;
import java.util.Objects;
import java.util.Optional;
import javax.annotation.Nullable;


public final class GetPackFilterArgs extends com.pulumi.resources.ResourceArgs {

    public static final GetPackFilterArgs Empty = new GetPackFilterArgs();

    /**
     * Specify the deployment environment for the certificate packs.
     * Available values: &#34;staging&#34;, &#34;production&#34;.
     * 
     */
    @Import(name="deploy")
    private @Nullable Output<String> deploy;

    /**
     * @return Specify the deployment environment for the certificate packs.
     * Available values: &#34;staging&#34;, &#34;production&#34;.
     * 
     */
    public Optional<Output<String>> deploy() {
        return Optional.ofNullable(this.deploy);
    }

    /**
     * Include Certificate Packs of all statuses, not just active ones.
     * Available values: &#34;all&#34;.
     * 
     */
    @Import(name="status")
    private @Nullable Output<String> status;

    /**
     * @return Include Certificate Packs of all statuses, not just active ones.
     * Available values: &#34;all&#34;.
     * 
     */
    public Optional<Output<String>> status() {
        return Optional.ofNullable(this.status);
    }

    private GetPackFilterArgs() {}

    private GetPackFilterArgs(GetPackFilterArgs $) {
        this.deploy = $.deploy;
        this.status = $.status;
    }

    public static Builder builder() {
        return new Builder();
    }
    public static Builder builder(GetPackFilterArgs defaults) {
        return new Builder(defaults);
    }

    public static final class Builder {
        private GetPackFilterArgs $;

        public Builder() {
            $ = new GetPackFilterArgs();
        }

        public Builder(GetPackFilterArgs defaults) {
            $ = new GetPackFilterArgs(Objects.requireNonNull(defaults));
        }

        /**
         * @param deploy Specify the deployment environment for the certificate packs.
         * Available values: &#34;staging&#34;, &#34;production&#34;.
         * 
         * @return builder
         * 
         */
        public Builder deploy(@Nullable Output<String> deploy) {
            $.deploy = deploy;
            return this;
        }

        /**
         * @param deploy Specify the deployment environment for the certificate packs.
         * Available values: &#34;staging&#34;, &#34;production&#34;.
         * 
         * @return builder
         * 
         */
        public Builder deploy(String deploy) {
            return deploy(Output.of(deploy));
        }

        /**
         * @param status Include Certificate Packs of all statuses, not just active ones.
         * Available values: &#34;all&#34;.
         * 
         * @return builder
         * 
         */
        public Builder status(@Nullable Output<String> status) {
            $.status = status;
            return this;
        }

        /**
         * @param status Include Certificate Packs of all statuses, not just active ones.
         * Available values: &#34;all&#34;.
         * 
         * @return builder
         * 
         */
        public Builder status(String status) {
            return status(Output.of(status));
        }

        public GetPackFilterArgs build() {
            return $;
        }
    }

}
