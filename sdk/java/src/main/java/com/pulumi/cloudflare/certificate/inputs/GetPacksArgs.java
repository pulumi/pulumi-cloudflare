// *** WARNING: this file was generated by pulumi-language-java. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.pulumi.cloudflare.certificate.inputs;

import com.pulumi.core.Output;
import com.pulumi.core.annotations.Import;
import java.lang.Integer;
import java.lang.String;
import java.util.Objects;
import java.util.Optional;
import javax.annotation.Nullable;


public final class GetPacksArgs extends com.pulumi.resources.InvokeArgs {

    public static final GetPacksArgs Empty = new GetPacksArgs();

    /**
     * Specify the deployment environment for the certificate packs.
     * Available values: &#34;staging&#34;, &#34;production&#34;.
     * 
     */
    @Import(name="deploy")
    private @Nullable Output<String> deploy;

    /**
     * @return Specify the deployment environment for the certificate packs.
     * Available values: &#34;staging&#34;, &#34;production&#34;.
     * 
     */
    public Optional<Output<String>> deploy() {
        return Optional.ofNullable(this.deploy);
    }

    /**
     * Max items to fetch, default: 1000
     * 
     */
    @Import(name="maxItems")
    private @Nullable Output<Integer> maxItems;

    /**
     * @return Max items to fetch, default: 1000
     * 
     */
    public Optional<Output<Integer>> maxItems() {
        return Optional.ofNullable(this.maxItems);
    }

    /**
     * Include Certificate Packs of all statuses, not just active ones.
     * Available values: &#34;all&#34;.
     * 
     */
    @Import(name="status")
    private @Nullable Output<String> status;

    /**
     * @return Include Certificate Packs of all statuses, not just active ones.
     * Available values: &#34;all&#34;.
     * 
     */
    public Optional<Output<String>> status() {
        return Optional.ofNullable(this.status);
    }

    /**
     * Identifier.
     * 
     */
    @Import(name="zoneId")
    private @Nullable Output<String> zoneId;

    /**
     * @return Identifier.
     * 
     */
    public Optional<Output<String>> zoneId() {
        return Optional.ofNullable(this.zoneId);
    }

    private GetPacksArgs() {}

    private GetPacksArgs(GetPacksArgs $) {
        this.deploy = $.deploy;
        this.maxItems = $.maxItems;
        this.status = $.status;
        this.zoneId = $.zoneId;
    }

    public static Builder builder() {
        return new Builder();
    }
    public static Builder builder(GetPacksArgs defaults) {
        return new Builder(defaults);
    }

    public static final class Builder {
        private GetPacksArgs $;

        public Builder() {
            $ = new GetPacksArgs();
        }

        public Builder(GetPacksArgs defaults) {
            $ = new GetPacksArgs(Objects.requireNonNull(defaults));
        }

        /**
         * @param deploy Specify the deployment environment for the certificate packs.
         * Available values: &#34;staging&#34;, &#34;production&#34;.
         * 
         * @return builder
         * 
         */
        public Builder deploy(@Nullable Output<String> deploy) {
            $.deploy = deploy;
            return this;
        }

        /**
         * @param deploy Specify the deployment environment for the certificate packs.
         * Available values: &#34;staging&#34;, &#34;production&#34;.
         * 
         * @return builder
         * 
         */
        public Builder deploy(String deploy) {
            return deploy(Output.of(deploy));
        }

        /**
         * @param maxItems Max items to fetch, default: 1000
         * 
         * @return builder
         * 
         */
        public Builder maxItems(@Nullable Output<Integer> maxItems) {
            $.maxItems = maxItems;
            return this;
        }

        /**
         * @param maxItems Max items to fetch, default: 1000
         * 
         * @return builder
         * 
         */
        public Builder maxItems(Integer maxItems) {
            return maxItems(Output.of(maxItems));
        }

        /**
         * @param status Include Certificate Packs of all statuses, not just active ones.
         * Available values: &#34;all&#34;.
         * 
         * @return builder
         * 
         */
        public Builder status(@Nullable Output<String> status) {
            $.status = status;
            return this;
        }

        /**
         * @param status Include Certificate Packs of all statuses, not just active ones.
         * Available values: &#34;all&#34;.
         * 
         * @return builder
         * 
         */
        public Builder status(String status) {
            return status(Output.of(status));
        }

        /**
         * @param zoneId Identifier.
         * 
         * @return builder
         * 
         */
        public Builder zoneId(@Nullable Output<String> zoneId) {
            $.zoneId = zoneId;
            return this;
        }

        /**
         * @param zoneId Identifier.
         * 
         * @return builder
         * 
         */
        public Builder zoneId(String zoneId) {
            return zoneId(Output.of(zoneId));
        }

        public GetPacksArgs build() {
            return $;
        }
    }

}
