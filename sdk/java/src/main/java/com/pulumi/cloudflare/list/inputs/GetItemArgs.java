// *** WARNING: this file was generated by pulumi-language-java. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.pulumi.cloudflare.list.inputs;

import com.pulumi.core.Output;
import com.pulumi.core.annotations.Import;
import com.pulumi.exceptions.MissingRequiredPropertyException;
import java.lang.String;
import java.util.Objects;
import java.util.Optional;
import javax.annotation.Nullable;


public final class GetItemArgs extends com.pulumi.resources.InvokeArgs {

    public static final GetItemArgs Empty = new GetItemArgs();

    /**
     * The Account ID for this resource.
     * 
     */
    @Import(name="accountId")
    private @Nullable Output<String> accountId;

    /**
     * @return The Account ID for this resource.
     * 
     */
    public Optional<Output<String>> accountId() {
        return Optional.ofNullable(this.accountId);
    }

    /**
     * Defines the unique ID of the item in the List.
     * 
     */
    @Import(name="itemId", required=true)
    private Output<String> itemId;

    /**
     * @return Defines the unique ID of the item in the List.
     * 
     */
    public Output<String> itemId() {
        return this.itemId;
    }

    /**
     * The unique ID of the list.
     * 
     */
    @Import(name="listId", required=true)
    private Output<String> listId;

    /**
     * @return The unique ID of the list.
     * 
     */
    public Output<String> listId() {
        return this.listId;
    }

    private GetItemArgs() {}

    private GetItemArgs(GetItemArgs $) {
        this.accountId = $.accountId;
        this.itemId = $.itemId;
        this.listId = $.listId;
    }

    public static Builder builder() {
        return new Builder();
    }
    public static Builder builder(GetItemArgs defaults) {
        return new Builder(defaults);
    }

    public static final class Builder {
        private GetItemArgs $;

        public Builder() {
            $ = new GetItemArgs();
        }

        public Builder(GetItemArgs defaults) {
            $ = new GetItemArgs(Objects.requireNonNull(defaults));
        }

        /**
         * @param accountId The Account ID for this resource.
         * 
         * @return builder
         * 
         */
        public Builder accountId(@Nullable Output<String> accountId) {
            $.accountId = accountId;
            return this;
        }

        /**
         * @param accountId The Account ID for this resource.
         * 
         * @return builder
         * 
         */
        public Builder accountId(String accountId) {
            return accountId(Output.of(accountId));
        }

        /**
         * @param itemId Defines the unique ID of the item in the List.
         * 
         * @return builder
         * 
         */
        public Builder itemId(Output<String> itemId) {
            $.itemId = itemId;
            return this;
        }

        /**
         * @param itemId Defines the unique ID of the item in the List.
         * 
         * @return builder
         * 
         */
        public Builder itemId(String itemId) {
            return itemId(Output.of(itemId));
        }

        /**
         * @param listId The unique ID of the list.
         * 
         * @return builder
         * 
         */
        public Builder listId(Output<String> listId) {
            $.listId = listId;
            return this;
        }

        /**
         * @param listId The unique ID of the list.
         * 
         * @return builder
         * 
         */
        public Builder listId(String listId) {
            return listId(Output.of(listId));
        }

        public GetItemArgs build() {
            if ($.itemId == null) {
                throw new MissingRequiredPropertyException("GetItemArgs", "itemId");
            }
            if ($.listId == null) {
                throw new MissingRequiredPropertyException("GetItemArgs", "listId");
            }
            return $;
        }
    }

}
