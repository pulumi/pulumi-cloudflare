// *** WARNING: this file was generated by pulumi-language-java. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.pulumi.cloudflare.secretsStore.inputs;

import com.pulumi.core.Output;
import com.pulumi.core.annotations.Import;
import com.pulumi.exceptions.MissingRequiredPropertyException;
import java.lang.String;
import java.util.List;
import java.util.Objects;
import java.util.Optional;
import javax.annotation.Nullable;


public final class GetSecretFilterArgs extends com.pulumi.resources.ResourceArgs {

    public static final GetSecretFilterArgs Empty = new GetSecretFilterArgs();

    /**
     * Direction to sort objects.
     * Available values: &#34;asc&#34;, &#34;desc&#34;.
     * 
     */
    @Import(name="direction", required=true)
    private Output<String> direction;

    /**
     * @return Direction to sort objects.
     * Available values: &#34;asc&#34;, &#34;desc&#34;.
     * 
     */
    public Output<String> direction() {
        return this.direction;
    }

    /**
     * Order secrets by values in the given field.
     * Available values: &#34;name&#34;, &#34;comment&#34;, &#34;created&#34;, &#34;modified&#34;, &#34;status&#34;.
     * 
     */
    @Import(name="order", required=true)
    private Output<String> order;

    /**
     * @return Order secrets by values in the given field.
     * Available values: &#34;name&#34;, &#34;comment&#34;, &#34;created&#34;, &#34;modified&#34;, &#34;status&#34;.
     * 
     */
    public Output<String> order() {
        return this.order;
    }

    /**
     * Only secrets with the given scopes will be returned.
     * 
     */
    @Import(name="scopes")
    private @Nullable Output<List<String>> scopes;

    /**
     * @return Only secrets with the given scopes will be returned.
     * 
     */
    public Optional<Output<List<String>>> scopes() {
        return Optional.ofNullable(this.scopes);
    }

    /**
     * Search secrets using a filter string, filtering across name and comment.
     * 
     */
    @Import(name="search")
    private @Nullable Output<String> search;

    /**
     * @return Search secrets using a filter string, filtering across name and comment.
     * 
     */
    public Optional<Output<String>> search() {
        return Optional.ofNullable(this.search);
    }

    private GetSecretFilterArgs() {}

    private GetSecretFilterArgs(GetSecretFilterArgs $) {
        this.direction = $.direction;
        this.order = $.order;
        this.scopes = $.scopes;
        this.search = $.search;
    }

    public static Builder builder() {
        return new Builder();
    }
    public static Builder builder(GetSecretFilterArgs defaults) {
        return new Builder(defaults);
    }

    public static final class Builder {
        private GetSecretFilterArgs $;

        public Builder() {
            $ = new GetSecretFilterArgs();
        }

        public Builder(GetSecretFilterArgs defaults) {
            $ = new GetSecretFilterArgs(Objects.requireNonNull(defaults));
        }

        /**
         * @param direction Direction to sort objects.
         * Available values: &#34;asc&#34;, &#34;desc&#34;.
         * 
         * @return builder
         * 
         */
        public Builder direction(Output<String> direction) {
            $.direction = direction;
            return this;
        }

        /**
         * @param direction Direction to sort objects.
         * Available values: &#34;asc&#34;, &#34;desc&#34;.
         * 
         * @return builder
         * 
         */
        public Builder direction(String direction) {
            return direction(Output.of(direction));
        }

        /**
         * @param order Order secrets by values in the given field.
         * Available values: &#34;name&#34;, &#34;comment&#34;, &#34;created&#34;, &#34;modified&#34;, &#34;status&#34;.
         * 
         * @return builder
         * 
         */
        public Builder order(Output<String> order) {
            $.order = order;
            return this;
        }

        /**
         * @param order Order secrets by values in the given field.
         * Available values: &#34;name&#34;, &#34;comment&#34;, &#34;created&#34;, &#34;modified&#34;, &#34;status&#34;.
         * 
         * @return builder
         * 
         */
        public Builder order(String order) {
            return order(Output.of(order));
        }

        /**
         * @param scopes Only secrets with the given scopes will be returned.
         * 
         * @return builder
         * 
         */
        public Builder scopes(@Nullable Output<List<String>> scopes) {
            $.scopes = scopes;
            return this;
        }

        /**
         * @param scopes Only secrets with the given scopes will be returned.
         * 
         * @return builder
         * 
         */
        public Builder scopes(List<String> scopes) {
            return scopes(Output.of(scopes));
        }

        /**
         * @param scopes Only secrets with the given scopes will be returned.
         * 
         * @return builder
         * 
         */
        public Builder scopes(String... scopes) {
            return scopes(List.of(scopes));
        }

        /**
         * @param search Search secrets using a filter string, filtering across name and comment.
         * 
         * @return builder
         * 
         */
        public Builder search(@Nullable Output<String> search) {
            $.search = search;
            return this;
        }

        /**
         * @param search Search secrets using a filter string, filtering across name and comment.
         * 
         * @return builder
         * 
         */
        public Builder search(String search) {
            return search(Output.of(search));
        }

        public GetSecretFilterArgs build() {
            if ($.direction == null) {
                throw new MissingRequiredPropertyException("GetSecretFilterArgs", "direction");
            }
            if ($.order == null) {
                throw new MissingRequiredPropertyException("GetSecretFilterArgs", "order");
            }
            return $;
        }
    }

}
