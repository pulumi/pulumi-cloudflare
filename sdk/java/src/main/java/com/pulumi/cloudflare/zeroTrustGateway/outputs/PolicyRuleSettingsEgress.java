// *** WARNING: this file was generated by pulumi-language-java. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.pulumi.cloudflare.zeroTrustGateway.outputs;

import com.pulumi.core.annotations.CustomType;
import java.lang.String;
import java.util.Objects;
import java.util.Optional;
import javax.annotation.Nullable;

@CustomType
public final class PolicyRuleSettingsEgress {
    /**
     * @return Specify the IPv4 address to use for egress.
     * 
     */
    private @Nullable String ipv4;
    /**
     * @return Specify the fallback IPv4 address to use for egress when the primary IPv4 fails. Set &#39;0.0.0.0&#39; to indicate local egress via WARP IPs.
     * 
     */
    private @Nullable String ipv4Fallback;
    /**
     * @return Specify the IPv6 range to use for egress.
     * 
     */
    private @Nullable String ipv6;

    private PolicyRuleSettingsEgress() {}
    /**
     * @return Specify the IPv4 address to use for egress.
     * 
     */
    public Optional<String> ipv4() {
        return Optional.ofNullable(this.ipv4);
    }
    /**
     * @return Specify the fallback IPv4 address to use for egress when the primary IPv4 fails. Set &#39;0.0.0.0&#39; to indicate local egress via WARP IPs.
     * 
     */
    public Optional<String> ipv4Fallback() {
        return Optional.ofNullable(this.ipv4Fallback);
    }
    /**
     * @return Specify the IPv6 range to use for egress.
     * 
     */
    public Optional<String> ipv6() {
        return Optional.ofNullable(this.ipv6);
    }

    public static Builder builder() {
        return new Builder();
    }

    public static Builder builder(PolicyRuleSettingsEgress defaults) {
        return new Builder(defaults);
    }
    @CustomType.Builder
    public static final class Builder {
        private @Nullable String ipv4;
        private @Nullable String ipv4Fallback;
        private @Nullable String ipv6;
        public Builder() {}
        public Builder(PolicyRuleSettingsEgress defaults) {
    	      Objects.requireNonNull(defaults);
    	      this.ipv4 = defaults.ipv4;
    	      this.ipv4Fallback = defaults.ipv4Fallback;
    	      this.ipv6 = defaults.ipv6;
        }

        @CustomType.Setter
        public Builder ipv4(@Nullable String ipv4) {

            this.ipv4 = ipv4;
            return this;
        }
        @CustomType.Setter
        public Builder ipv4Fallback(@Nullable String ipv4Fallback) {

            this.ipv4Fallback = ipv4Fallback;
            return this;
        }
        @CustomType.Setter
        public Builder ipv6(@Nullable String ipv6) {

            this.ipv6 = ipv6;
            return this;
        }
        public PolicyRuleSettingsEgress build() {
            final var _resultValue = new PolicyRuleSettingsEgress();
            _resultValue.ipv4 = ipv4;
            _resultValue.ipv4Fallback = ipv4Fallback;
            _resultValue.ipv6 = ipv6;
            return _resultValue;
        }
    }
}
