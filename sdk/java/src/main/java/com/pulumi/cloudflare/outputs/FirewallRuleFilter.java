// *** WARNING: this file was generated by pulumi-language-java. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.pulumi.cloudflare.outputs;

import com.pulumi.core.annotations.CustomType;
import java.lang.Boolean;
import java.lang.String;
import java.util.Objects;
import java.util.Optional;
import javax.annotation.Nullable;

@CustomType
public final class FirewallRuleFilter {
    /**
     * @return An informative summary of the filter.
     * 
     */
    private @Nullable String description;
    /**
     * @return The filter expression. For more information, refer to [Expressions](https://developers.cloudflare.com/ruleset-engine/rules-language/expressions/).
     * 
     */
    private @Nullable String expression;
    /**
     * @return The unique identifier of the filter.
     * 
     */
    private @Nullable String id;
    /**
     * @return When true, indicates that the filter is currently paused.
     * 
     */
    private @Nullable Boolean paused;
    /**
     * @return A short reference tag. Allows you to select related filters.
     * 
     */
    private @Nullable String ref;

    private FirewallRuleFilter() {}
    /**
     * @return An informative summary of the filter.
     * 
     */
    public Optional<String> description() {
        return Optional.ofNullable(this.description);
    }
    /**
     * @return The filter expression. For more information, refer to [Expressions](https://developers.cloudflare.com/ruleset-engine/rules-language/expressions/).
     * 
     */
    public Optional<String> expression() {
        return Optional.ofNullable(this.expression);
    }
    /**
     * @return The unique identifier of the filter.
     * 
     */
    public Optional<String> id() {
        return Optional.ofNullable(this.id);
    }
    /**
     * @return When true, indicates that the filter is currently paused.
     * 
     */
    public Optional<Boolean> paused() {
        return Optional.ofNullable(this.paused);
    }
    /**
     * @return A short reference tag. Allows you to select related filters.
     * 
     */
    public Optional<String> ref() {
        return Optional.ofNullable(this.ref);
    }

    public static Builder builder() {
        return new Builder();
    }

    public static Builder builder(FirewallRuleFilter defaults) {
        return new Builder(defaults);
    }
    @CustomType.Builder
    public static final class Builder {
        private @Nullable String description;
        private @Nullable String expression;
        private @Nullable String id;
        private @Nullable Boolean paused;
        private @Nullable String ref;
        public Builder() {}
        public Builder(FirewallRuleFilter defaults) {
    	      Objects.requireNonNull(defaults);
    	      this.description = defaults.description;
    	      this.expression = defaults.expression;
    	      this.id = defaults.id;
    	      this.paused = defaults.paused;
    	      this.ref = defaults.ref;
        }

        @CustomType.Setter
        public Builder description(@Nullable String description) {

            this.description = description;
            return this;
        }
        @CustomType.Setter
        public Builder expression(@Nullable String expression) {

            this.expression = expression;
            return this;
        }
        @CustomType.Setter
        public Builder id(@Nullable String id) {

            this.id = id;
            return this;
        }
        @CustomType.Setter
        public Builder paused(@Nullable Boolean paused) {

            this.paused = paused;
            return this;
        }
        @CustomType.Setter
        public Builder ref(@Nullable String ref) {

            this.ref = ref;
            return this;
        }
        public FirewallRuleFilter build() {
            final var _resultValue = new FirewallRuleFilter();
            _resultValue.description = description;
            _resultValue.expression = expression;
            _resultValue.id = id;
            _resultValue.paused = paused;
            _resultValue.ref = ref;
            return _resultValue;
        }
    }
}
