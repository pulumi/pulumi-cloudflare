// *** WARNING: this file was generated by pulumi-language-java. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.pulumi.cloudflare.certificate.inputs;

import com.pulumi.core.annotations.Import;
import java.lang.String;
import java.util.Objects;
import java.util.Optional;
import javax.annotation.Nullable;


public final class GetPackFilter extends com.pulumi.resources.InvokeArgs {

    public static final GetPackFilter Empty = new GetPackFilter();

    /**
     * Specify the deployment environment for the certificate packs.
     * Available values: &#34;staging&#34;, &#34;production&#34;.
     * 
     */
    @Import(name="deploy")
    private @Nullable String deploy;

    /**
     * @return Specify the deployment environment for the certificate packs.
     * Available values: &#34;staging&#34;, &#34;production&#34;.
     * 
     */
    public Optional<String> deploy() {
        return Optional.ofNullable(this.deploy);
    }

    /**
     * Include Certificate Packs of all statuses, not just active ones.
     * Available values: &#34;all&#34;.
     * 
     */
    @Import(name="status")
    private @Nullable String status;

    /**
     * @return Include Certificate Packs of all statuses, not just active ones.
     * Available values: &#34;all&#34;.
     * 
     */
    public Optional<String> status() {
        return Optional.ofNullable(this.status);
    }

    private GetPackFilter() {}

    private GetPackFilter(GetPackFilter $) {
        this.deploy = $.deploy;
        this.status = $.status;
    }

    public static Builder builder() {
        return new Builder();
    }
    public static Builder builder(GetPackFilter defaults) {
        return new Builder(defaults);
    }

    public static final class Builder {
        private GetPackFilter $;

        public Builder() {
            $ = new GetPackFilter();
        }

        public Builder(GetPackFilter defaults) {
            $ = new GetPackFilter(Objects.requireNonNull(defaults));
        }

        /**
         * @param deploy Specify the deployment environment for the certificate packs.
         * Available values: &#34;staging&#34;, &#34;production&#34;.
         * 
         * @return builder
         * 
         */
        public Builder deploy(@Nullable String deploy) {
            $.deploy = deploy;
            return this;
        }

        /**
         * @param status Include Certificate Packs of all statuses, not just active ones.
         * Available values: &#34;all&#34;.
         * 
         * @return builder
         * 
         */
        public Builder status(@Nullable String status) {
            $.status = status;
            return this;
        }

        public GetPackFilter build() {
            return $;
        }
    }

}
