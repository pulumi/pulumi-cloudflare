// *** WARNING: this file was generated by pulumi-language-java. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.pulumi.cloudflare.inputs;

import com.pulumi.core.Output;
import com.pulumi.core.annotations.Import;
import com.pulumi.exceptions.MissingRequiredPropertyException;
import java.lang.String;
import java.util.Objects;
import java.util.Optional;
import javax.annotation.Nullable;


public final class GetCustomCsrArgs extends com.pulumi.resources.InvokeArgs {

    public static final GetCustomCsrArgs Empty = new GetCustomCsrArgs();

    /**
     * The Account ID to use for this endpoint. Mutually exclusive with the Zone ID.
     * 
     */
    @Import(name="accountId")
    private @Nullable Output<String> accountId;

    /**
     * @return The Account ID to use for this endpoint. Mutually exclusive with the Zone ID.
     * 
     */
    public Optional<Output<String>> accountId() {
        return Optional.ofNullable(this.accountId);
    }

    /**
     * Custom CSR identifier tag.
     * 
     */
    @Import(name="customCsrId", required=true)
    private Output<String> customCsrId;

    /**
     * @return Custom CSR identifier tag.
     * 
     */
    public Output<String> customCsrId() {
        return this.customCsrId;
    }

    /**
     * The Zone ID to use for this endpoint. Mutually exclusive with the Account ID.
     * 
     */
    @Import(name="zoneId")
    private @Nullable Output<String> zoneId;

    /**
     * @return The Zone ID to use for this endpoint. Mutually exclusive with the Account ID.
     * 
     */
    public Optional<Output<String>> zoneId() {
        return Optional.ofNullable(this.zoneId);
    }

    private GetCustomCsrArgs() {}

    private GetCustomCsrArgs(GetCustomCsrArgs $) {
        this.accountId = $.accountId;
        this.customCsrId = $.customCsrId;
        this.zoneId = $.zoneId;
    }

    public static Builder builder() {
        return new Builder();
    }
    public static Builder builder(GetCustomCsrArgs defaults) {
        return new Builder(defaults);
    }

    public static final class Builder {
        private GetCustomCsrArgs $;

        public Builder() {
            $ = new GetCustomCsrArgs();
        }

        public Builder(GetCustomCsrArgs defaults) {
            $ = new GetCustomCsrArgs(Objects.requireNonNull(defaults));
        }

        /**
         * @param accountId The Account ID to use for this endpoint. Mutually exclusive with the Zone ID.
         * 
         * @return builder
         * 
         */
        public Builder accountId(@Nullable Output<String> accountId) {
            $.accountId = accountId;
            return this;
        }

        /**
         * @param accountId The Account ID to use for this endpoint. Mutually exclusive with the Zone ID.
         * 
         * @return builder
         * 
         */
        public Builder accountId(String accountId) {
            return accountId(Output.of(accountId));
        }

        /**
         * @param customCsrId Custom CSR identifier tag.
         * 
         * @return builder
         * 
         */
        public Builder customCsrId(Output<String> customCsrId) {
            $.customCsrId = customCsrId;
            return this;
        }

        /**
         * @param customCsrId Custom CSR identifier tag.
         * 
         * @return builder
         * 
         */
        public Builder customCsrId(String customCsrId) {
            return customCsrId(Output.of(customCsrId));
        }

        /**
         * @param zoneId The Zone ID to use for this endpoint. Mutually exclusive with the Account ID.
         * 
         * @return builder
         * 
         */
        public Builder zoneId(@Nullable Output<String> zoneId) {
            $.zoneId = zoneId;
            return this;
        }

        /**
         * @param zoneId The Zone ID to use for this endpoint. Mutually exclusive with the Account ID.
         * 
         * @return builder
         * 
         */
        public Builder zoneId(String zoneId) {
            return zoneId(Output.of(zoneId));
        }

        public GetCustomCsrArgs build() {
            if ($.customCsrId == null) {
                throw new MissingRequiredPropertyException("GetCustomCsrArgs", "customCsrId");
            }
            return $;
        }
    }

}
