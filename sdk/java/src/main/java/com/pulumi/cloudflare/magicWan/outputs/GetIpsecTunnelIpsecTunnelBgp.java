// *** WARNING: this file was generated by pulumi-language-java. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.pulumi.cloudflare.magicWan.outputs;

import com.pulumi.core.annotations.CustomType;
import com.pulumi.exceptions.MissingRequiredPropertyException;
import java.lang.Integer;
import java.lang.String;
import java.util.List;
import java.util.Objects;

@CustomType
public final class GetIpsecTunnelIpsecTunnelBgp {
    /**
     * @return ASN used on the customer end of the BGP session
     * 
     */
    private Integer customerAsn;
    /**
     * @return Prefixes in this list will be advertised to the customer device, in addition to the routes in the Magic routing table.
     * 
     */
    private List<String> extraPrefixes;
    /**
     * @return MD5 key to use for session authentication.
     * 
     */
    private String md5Key;

    private GetIpsecTunnelIpsecTunnelBgp() {}
    /**
     * @return ASN used on the customer end of the BGP session
     * 
     */
    public Integer customerAsn() {
        return this.customerAsn;
    }
    /**
     * @return Prefixes in this list will be advertised to the customer device, in addition to the routes in the Magic routing table.
     * 
     */
    public List<String> extraPrefixes() {
        return this.extraPrefixes;
    }
    /**
     * @return MD5 key to use for session authentication.
     * 
     */
    public String md5Key() {
        return this.md5Key;
    }

    public static Builder builder() {
        return new Builder();
    }

    public static Builder builder(GetIpsecTunnelIpsecTunnelBgp defaults) {
        return new Builder(defaults);
    }
    @CustomType.Builder
    public static final class Builder {
        private Integer customerAsn;
        private List<String> extraPrefixes;
        private String md5Key;
        public Builder() {}
        public Builder(GetIpsecTunnelIpsecTunnelBgp defaults) {
    	      Objects.requireNonNull(defaults);
    	      this.customerAsn = defaults.customerAsn;
    	      this.extraPrefixes = defaults.extraPrefixes;
    	      this.md5Key = defaults.md5Key;
        }

        @CustomType.Setter
        public Builder customerAsn(Integer customerAsn) {
            if (customerAsn == null) {
              throw new MissingRequiredPropertyException("GetIpsecTunnelIpsecTunnelBgp", "customerAsn");
            }
            this.customerAsn = customerAsn;
            return this;
        }
        @CustomType.Setter
        public Builder extraPrefixes(List<String> extraPrefixes) {
            if (extraPrefixes == null) {
              throw new MissingRequiredPropertyException("GetIpsecTunnelIpsecTunnelBgp", "extraPrefixes");
            }
            this.extraPrefixes = extraPrefixes;
            return this;
        }
        public Builder extraPrefixes(String... extraPrefixes) {
            return extraPrefixes(List.of(extraPrefixes));
        }
        @CustomType.Setter
        public Builder md5Key(String md5Key) {
            if (md5Key == null) {
              throw new MissingRequiredPropertyException("GetIpsecTunnelIpsecTunnelBgp", "md5Key");
            }
            this.md5Key = md5Key;
            return this;
        }
        public GetIpsecTunnelIpsecTunnelBgp build() {
            final var _resultValue = new GetIpsecTunnelIpsecTunnelBgp();
            _resultValue.customerAsn = customerAsn;
            _resultValue.extraPrefixes = extraPrefixes;
            _resultValue.md5Key = md5Key;
            return _resultValue;
        }
    }
}
