// *** WARNING: this file was generated by pulumi-language-java. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.pulumi.cloudflare.zeroTrustDns.outputs;

import com.pulumi.cloudflare.zeroTrustDns.outputs.LocationEndpointsDohNetwork;
import com.pulumi.core.annotations.CustomType;
import java.lang.Boolean;
import java.util.List;
import java.util.Objects;
import java.util.Optional;
import javax.annotation.Nullable;

@CustomType
public final class LocationEndpointsDoh {
    /**
     * @return Indicate whether the DOH endpoint is enabled for this location.
     * 
     */
    private @Nullable Boolean enabled;
    /**
     * @return Specify the list of allowed source IP network ranges for this endpoint. When the list is empty, the endpoint allows all source IPs. The list takes effect only if the endpoint is enabled for this location.
     * 
     */
    private @Nullable List<LocationEndpointsDohNetwork> networks;
    /**
     * @return Specify whether the DOH endpoint requires user identity authentication.
     * 
     */
    private @Nullable Boolean requireToken;

    private LocationEndpointsDoh() {}
    /**
     * @return Indicate whether the DOH endpoint is enabled for this location.
     * 
     */
    public Optional<Boolean> enabled() {
        return Optional.ofNullable(this.enabled);
    }
    /**
     * @return Specify the list of allowed source IP network ranges for this endpoint. When the list is empty, the endpoint allows all source IPs. The list takes effect only if the endpoint is enabled for this location.
     * 
     */
    public List<LocationEndpointsDohNetwork> networks() {
        return this.networks == null ? List.of() : this.networks;
    }
    /**
     * @return Specify whether the DOH endpoint requires user identity authentication.
     * 
     */
    public Optional<Boolean> requireToken() {
        return Optional.ofNullable(this.requireToken);
    }

    public static Builder builder() {
        return new Builder();
    }

    public static Builder builder(LocationEndpointsDoh defaults) {
        return new Builder(defaults);
    }
    @CustomType.Builder
    public static final class Builder {
        private @Nullable Boolean enabled;
        private @Nullable List<LocationEndpointsDohNetwork> networks;
        private @Nullable Boolean requireToken;
        public Builder() {}
        public Builder(LocationEndpointsDoh defaults) {
    	      Objects.requireNonNull(defaults);
    	      this.enabled = defaults.enabled;
    	      this.networks = defaults.networks;
    	      this.requireToken = defaults.requireToken;
        }

        @CustomType.Setter
        public Builder enabled(@Nullable Boolean enabled) {

            this.enabled = enabled;
            return this;
        }
        @CustomType.Setter
        public Builder networks(@Nullable List<LocationEndpointsDohNetwork> networks) {

            this.networks = networks;
            return this;
        }
        public Builder networks(LocationEndpointsDohNetwork... networks) {
            return networks(List.of(networks));
        }
        @CustomType.Setter
        public Builder requireToken(@Nullable Boolean requireToken) {

            this.requireToken = requireToken;
            return this;
        }
        public LocationEndpointsDoh build() {
            final var _resultValue = new LocationEndpointsDoh();
            _resultValue.enabled = enabled;
            _resultValue.networks = networks;
            _resultValue.requireToken = requireToken;
            return _resultValue;
        }
    }
}
