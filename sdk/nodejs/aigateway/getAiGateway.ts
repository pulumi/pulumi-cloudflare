// *** WARNING: this file was generated by pulumi-language-nodejs. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

import * as pulumi from "@pulumi/pulumi";
import * as inputs from "../types/input";
import * as outputs from "../types/output";
import * as utilities from "../utilities";

/**
 * Accepted Permissions
 *
 * - `AI Gateway Read`
 * - `AI Gateway Write`
 *
 * ## Example Usage
 *
 * ```typescript
 * import * as pulumi from "@pulumi/pulumi";
 * import * as cloudflare from "@pulumi/cloudflare";
 *
 * const exampleAiGateway = cloudflare.aigateway.getAiGateway({
 *     accountId: "3ebbcb006d4d46d7bb6a8c7f14676cb0",
 *     id: "my-gateway",
 * });
 * ```
 */
export function getAiGateway(args?: GetAiGatewayArgs, opts?: pulumi.InvokeOptions): Promise<GetAiGatewayResult> {
    args = args || {};
    opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts || {});
    return pulumi.runtime.invoke("cloudflare:aiGateway/aiGateway:getAiGateway", {
        "accountId": args.accountId,
        "filter": args.filter,
        "id": args.id,
    }, opts);
}

/**
 * A collection of arguments for invoking getAiGateway.
 */
export interface GetAiGatewayArgs {
    accountId?: string;
    filter?: inputs.aiGateway.GetAiGatewayFilter;
    /**
     * gateway id
     */
    id?: string;
}

/**
 * A collection of values returned by getAiGateway.
 */
export interface GetAiGatewayResult {
    readonly accountId?: string;
    readonly authentication: boolean;
    readonly cacheInvalidateOnUpdate: boolean;
    readonly cacheTtl: number;
    readonly collectLogs: boolean;
    readonly createdAt: string;
    readonly dlp: outputs.aiGateway.GetAiGatewayDlp;
    readonly filter?: outputs.aiGateway.GetAiGatewayFilter;
    readonly guardrails: outputs.aiGateway.GetAiGatewayGuardrails;
    /**
     * gateway id
     */
    readonly id: string;
    readonly isDefault: boolean;
    readonly logManagement: number;
    /**
     * Available values: "STOP*INSERTING", "DELETE*OLDEST".
     */
    readonly logManagementStrategy: string;
    readonly logpush: boolean;
    readonly logpushPublicKey: string;
    readonly modifiedAt: string;
    readonly otels: outputs.aiGateway.GetAiGatewayOtel[];
    readonly rateLimitingInterval: number;
    readonly rateLimitingLimit: number;
    /**
     * Available values: "fixed", "sliding".
     */
    readonly rateLimitingTechnique: string;
    /**
     * Backoff strategy for retry delays
     * Available values: "constant", "linear", "exponential".
     */
    readonly retryBackoff: string;
    /**
     * Delay between retry attempts in milliseconds (0-5000)
     */
    readonly retryDelay: number;
    /**
     * Maximum number of retry attempts for failed requests (1-5)
     */
    readonly retryMaxAttempts: number;
    readonly spendLimits: outputs.aiGateway.GetAiGatewaySpendLimits;
    readonly storeId: string;
    readonly stripe: outputs.aiGateway.GetAiGatewayStripe;
    /**
     * Controls how Workers AI inference calls routed through this gateway are billed. Only 'postpaid' is currently supported.
     * Available values: "postpaid".
     */
    readonly workersAiBillingMode: string;
    readonly zdr: boolean;
}
/**
 * Accepted Permissions
 *
 * - `AI Gateway Read`
 * - `AI Gateway Write`
 *
 * ## Example Usage
 *
 * ```typescript
 * import * as pulumi from "@pulumi/pulumi";
 * import * as cloudflare from "@pulumi/cloudflare";
 *
 * const exampleAiGateway = cloudflare.aigateway.getAiGateway({
 *     accountId: "3ebbcb006d4d46d7bb6a8c7f14676cb0",
 *     id: "my-gateway",
 * });
 * ```
 */
export function getAiGatewayOutput(args?: GetAiGatewayOutputArgs, opts?: pulumi.InvokeOutputOptions): pulumi.Output<GetAiGatewayResult> {
    args = args || {};
    opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts || {});
    return pulumi.runtime.invokeOutput("cloudflare:aiGateway/aiGateway:getAiGateway", {
        "accountId": args.accountId,
        "filter": args.filter,
        "id": args.id,
    }, opts);
}

/**
 * A collection of arguments for invoking getAiGateway.
 */
export interface GetAiGatewayOutputArgs {
    accountId?: pulumi.Input<string | undefined>;
    filter?: pulumi.Input<inputs.aiGateway.GetAiGatewayFilterArgs | undefined>;
    /**
     * gateway id
     */
    id?: pulumi.Input<string | undefined>;
}
