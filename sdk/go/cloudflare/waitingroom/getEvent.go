// Code generated by pulumi-language-go DO NOT EDIT.
// *** WARNING: Do not edit by hand unless you're certain you know what you are doing! ***

package waitingroom

import (
	"context"
	"reflect"

	"github.com/pulumi/pulumi-cloudflare/sdk/v6/go/cloudflare/internal"
	"github.com/pulumi/pulumi/sdk/v3/go/pulumi"
)

// Accepted Permissions
//
// - `Waiting Rooms Read`
// - `Waiting Rooms Write`
//
// ## Example Usage
//
// ```go
// package main
//
// import (
//
//	"github.com/pulumi/pulumi-cloudflare/sdk/v6/go/cloudflare/waitingroom"
//	"github.com/pulumi/pulumi/sdk/v3/go/pulumi"
//
// )
//
//	func main() {
//		pulumi.Run(func(ctx *pulumi.Context) error {
//			_, err := waitingroom.GetEvent(ctx, &waitingroom.GetEventArgs{
//				ZoneId:        pulumi.StringRef("023e105f4ecef8ad9ca31a8372d0c353"),
//				WaitingRoomId: "699d98642c564d2e855e9661899b7252",
//				EventId:       "25756b2dfe6e378a06b033b670413757",
//			}, nil)
//			if err != nil {
//				return err
//			}
//			return nil
//		})
//	}
//
// ```
func LookupEvent(ctx *pulumi.Context, args *LookupEventArgs, opts ...pulumi.InvokeOption) (*LookupEventResult, error) {
	opts = internal.PkgInvokeDefaultOpts(opts)
	var rv LookupEventResult
	err := ctx.Invoke("cloudflare:waitingRoom/event:getEvent", args, &rv, opts...)
	if err != nil {
		return nil, err
	}
	return &rv, nil
}

// A collection of arguments for invoking getEvent.
type LookupEventArgs struct {
	EventId       string `pulumi:"eventId"`
	WaitingRoomId string `pulumi:"waitingRoomId"`
	// Identifier.
	ZoneId *string `pulumi:"zoneId"`
}

// A collection of values returned by getEvent.
type LookupEventResult struct {
	CreatedOn string `pulumi:"createdOn"`
	// If set, the event will override the waiting room's `customPageHtml` property while it is active. If null, the event will inherit it.
	CustomPageHtml string `pulumi:"customPageHtml"`
	// A note that you can use to add more details about the event.
	Description string `pulumi:"description"`
	// If set, the event will override the waiting room's `disableSessionRenewal` property while it is active. If null, the event will inherit it.
	DisableSessionRenewal bool `pulumi:"disableSessionRenewal"`
	// An ISO 8601 timestamp that marks the end of the event.
	EventEndTime string `pulumi:"eventEndTime"`
	EventId      string `pulumi:"eventId"`
	// An ISO 8601 timestamp that marks the start of the event. At this time, queued users will be processed with the event's configuration. The start time must be at least one minute before `eventEndTime`.
	EventStartTime string `pulumi:"eventStartTime"`
	// The ID of this resource.
	Id         string `pulumi:"id"`
	ModifiedOn string `pulumi:"modifiedOn"`
	// A unique name to identify the event. Only alphanumeric characters, hyphens and underscores are allowed.
	Name string `pulumi:"name"`
	// If set, the event will override the waiting room's `newUsersPerMinute` property while it is active. If null, the event will inherit it. This can only be set if the event's `totalActiveUsers` property is also set.
	NewUsersPerMinute int `pulumi:"newUsersPerMinute"`
	// An ISO 8601 timestamp that marks when to begin queueing all users before the event starts. The prequeue must start at least five minutes before `eventStartTime`.
	PrequeueStartTime string `pulumi:"prequeueStartTime"`
	// If set, the event will override the waiting room's `queueingMethod` property while it is active. If null, the event will inherit it.
	QueueingMethod string `pulumi:"queueingMethod"`
	// If set, the event will override the waiting room's `sessionDuration` property while it is active. If null, the event will inherit it.
	SessionDuration int `pulumi:"sessionDuration"`
	// If enabled, users in the prequeue will be shuffled randomly at the `eventStartTime`. Requires that `prequeueStartTime` is not null. This is useful for situations when many users will join the event prequeue at the same time and you want to shuffle them to ensure fairness. Naturally, it makes the most sense to enable this feature when the `queueingMethod` during the event respects ordering such as **fifo**, or else the shuffling may be unnecessary.
	ShuffleAtEventStart bool `pulumi:"shuffleAtEventStart"`
	// Suspends or allows an event. If set to `true`, the event is ignored and traffic will be handled based on the waiting room configuration.
	Suspended bool `pulumi:"suspended"`
	// If set, the event will override the waiting room's `totalActiveUsers` property while it is active. If null, the event will inherit it. This can only be set if the event's `newUsersPerMinute` property is also set.
	TotalActiveUsers int `pulumi:"totalActiveUsers"`
	// If set, the event will override the waiting room's `turnstileAction` property while it is active. If null, the event will inherit it.
	// Available values: "log", "infiniteQueue".
	TurnstileAction string `pulumi:"turnstileAction"`
	// If set, the event will override the waiting room's `turnstileMode` property while it is active. If null, the event will inherit it.
	// Available values: "off", "invisible", "visible*non*interactive", "visibleManaged".
	TurnstileMode string `pulumi:"turnstileMode"`
	WaitingRoomId string `pulumi:"waitingRoomId"`
	// Identifier.
	ZoneId *string `pulumi:"zoneId"`
}

func LookupEventOutput(ctx *pulumi.Context, args LookupEventOutputArgs, opts ...pulumi.InvokeOption) LookupEventResultOutput {
	return pulumi.ToOutputWithContext(ctx.Context(), args).
		ApplyT(func(v interface{}) (LookupEventResultOutput, error) {
			args := v.(LookupEventArgs)
			options := pulumi.InvokeOutputOptions{InvokeOptions: internal.PkgInvokeDefaultOpts(opts)}
			return ctx.InvokeOutput("cloudflare:waitingRoom/event:getEvent", args, LookupEventResultOutput{}, options).(LookupEventResultOutput), nil
		}).(LookupEventResultOutput)
}

// A collection of arguments for invoking getEvent.
type LookupEventOutputArgs struct {
	EventId       pulumi.StringInput `pulumi:"eventId"`
	WaitingRoomId pulumi.StringInput `pulumi:"waitingRoomId"`
	// Identifier.
	ZoneId pulumi.StringPtrInput `pulumi:"zoneId"`
}

func (LookupEventOutputArgs) ElementType() reflect.Type {
	return reflect.TypeOf((*LookupEventArgs)(nil)).Elem()
}

// A collection of values returned by getEvent.
type LookupEventResultOutput struct{ *pulumi.OutputState }

func (LookupEventResultOutput) ElementType() reflect.Type {
	return reflect.TypeOf((*LookupEventResult)(nil)).Elem()
}

func (o LookupEventResultOutput) ToLookupEventResultOutput() LookupEventResultOutput {
	return o
}

func (o LookupEventResultOutput) ToLookupEventResultOutputWithContext(ctx context.Context) LookupEventResultOutput {
	return o
}

func (o LookupEventResultOutput) CreatedOn() pulumi.StringOutput {
	return o.ApplyT(func(v LookupEventResult) string { return v.CreatedOn }).(pulumi.StringOutput)
}

// If set, the event will override the waiting room's `customPageHtml` property while it is active. If null, the event will inherit it.
func (o LookupEventResultOutput) CustomPageHtml() pulumi.StringOutput {
	return o.ApplyT(func(v LookupEventResult) string { return v.CustomPageHtml }).(pulumi.StringOutput)
}

// A note that you can use to add more details about the event.
func (o LookupEventResultOutput) Description() pulumi.StringOutput {
	return o.ApplyT(func(v LookupEventResult) string { return v.Description }).(pulumi.StringOutput)
}

// If set, the event will override the waiting room's `disableSessionRenewal` property while it is active. If null, the event will inherit it.
func (o LookupEventResultOutput) DisableSessionRenewal() pulumi.BoolOutput {
	return o.ApplyT(func(v LookupEventResult) bool { return v.DisableSessionRenewal }).(pulumi.BoolOutput)
}

// An ISO 8601 timestamp that marks the end of the event.
func (o LookupEventResultOutput) EventEndTime() pulumi.StringOutput {
	return o.ApplyT(func(v LookupEventResult) string { return v.EventEndTime }).(pulumi.StringOutput)
}

func (o LookupEventResultOutput) EventId() pulumi.StringOutput {
	return o.ApplyT(func(v LookupEventResult) string { return v.EventId }).(pulumi.StringOutput)
}

// An ISO 8601 timestamp that marks the start of the event. At this time, queued users will be processed with the event's configuration. The start time must be at least one minute before `eventEndTime`.
func (o LookupEventResultOutput) EventStartTime() pulumi.StringOutput {
	return o.ApplyT(func(v LookupEventResult) string { return v.EventStartTime }).(pulumi.StringOutput)
}

// The ID of this resource.
func (o LookupEventResultOutput) Id() pulumi.StringOutput {
	return o.ApplyT(func(v LookupEventResult) string { return v.Id }).(pulumi.StringOutput)
}

func (o LookupEventResultOutput) ModifiedOn() pulumi.StringOutput {
	return o.ApplyT(func(v LookupEventResult) string { return v.ModifiedOn }).(pulumi.StringOutput)
}

// A unique name to identify the event. Only alphanumeric characters, hyphens and underscores are allowed.
func (o LookupEventResultOutput) Name() pulumi.StringOutput {
	return o.ApplyT(func(v LookupEventResult) string { return v.Name }).(pulumi.StringOutput)
}

// If set, the event will override the waiting room's `newUsersPerMinute` property while it is active. If null, the event will inherit it. This can only be set if the event's `totalActiveUsers` property is also set.
func (o LookupEventResultOutput) NewUsersPerMinute() pulumi.IntOutput {
	return o.ApplyT(func(v LookupEventResult) int { return v.NewUsersPerMinute }).(pulumi.IntOutput)
}

// An ISO 8601 timestamp that marks when to begin queueing all users before the event starts. The prequeue must start at least five minutes before `eventStartTime`.
func (o LookupEventResultOutput) PrequeueStartTime() pulumi.StringOutput {
	return o.ApplyT(func(v LookupEventResult) string { return v.PrequeueStartTime }).(pulumi.StringOutput)
}

// If set, the event will override the waiting room's `queueingMethod` property while it is active. If null, the event will inherit it.
func (o LookupEventResultOutput) QueueingMethod() pulumi.StringOutput {
	return o.ApplyT(func(v LookupEventResult) string { return v.QueueingMethod }).(pulumi.StringOutput)
}

// If set, the event will override the waiting room's `sessionDuration` property while it is active. If null, the event will inherit it.
func (o LookupEventResultOutput) SessionDuration() pulumi.IntOutput {
	return o.ApplyT(func(v LookupEventResult) int { return v.SessionDuration }).(pulumi.IntOutput)
}

// If enabled, users in the prequeue will be shuffled randomly at the `eventStartTime`. Requires that `prequeueStartTime` is not null. This is useful for situations when many users will join the event prequeue at the same time and you want to shuffle them to ensure fairness. Naturally, it makes the most sense to enable this feature when the `queueingMethod` during the event respects ordering such as **fifo**, or else the shuffling may be unnecessary.
func (o LookupEventResultOutput) ShuffleAtEventStart() pulumi.BoolOutput {
	return o.ApplyT(func(v LookupEventResult) bool { return v.ShuffleAtEventStart }).(pulumi.BoolOutput)
}

// Suspends or allows an event. If set to `true`, the event is ignored and traffic will be handled based on the waiting room configuration.
func (o LookupEventResultOutput) Suspended() pulumi.BoolOutput {
	return o.ApplyT(func(v LookupEventResult) bool { return v.Suspended }).(pulumi.BoolOutput)
}

// If set, the event will override the waiting room's `totalActiveUsers` property while it is active. If null, the event will inherit it. This can only be set if the event's `newUsersPerMinute` property is also set.
func (o LookupEventResultOutput) TotalActiveUsers() pulumi.IntOutput {
	return o.ApplyT(func(v LookupEventResult) int { return v.TotalActiveUsers }).(pulumi.IntOutput)
}

// If set, the event will override the waiting room's `turnstileAction` property while it is active. If null, the event will inherit it.
// Available values: "log", "infiniteQueue".
func (o LookupEventResultOutput) TurnstileAction() pulumi.StringOutput {
	return o.ApplyT(func(v LookupEventResult) string { return v.TurnstileAction }).(pulumi.StringOutput)
}

// If set, the event will override the waiting room's `turnstileMode` property while it is active. If null, the event will inherit it.
// Available values: "off", "invisible", "visible*non*interactive", "visibleManaged".
func (o LookupEventResultOutput) TurnstileMode() pulumi.StringOutput {
	return o.ApplyT(func(v LookupEventResult) string { return v.TurnstileMode }).(pulumi.StringOutput)
}

func (o LookupEventResultOutput) WaitingRoomId() pulumi.StringOutput {
	return o.ApplyT(func(v LookupEventResult) string { return v.WaitingRoomId }).(pulumi.StringOutput)
}

// Identifier.
func (o LookupEventResultOutput) ZoneId() pulumi.StringPtrOutput {
	return o.ApplyT(func(v LookupEventResult) *string { return v.ZoneId }).(pulumi.StringPtrOutput)
}

func init() {
	pulumi.RegisterOutputType(LookupEventResultOutput{})
}
